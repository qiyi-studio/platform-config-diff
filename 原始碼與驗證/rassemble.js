...
const p1 = fs.readFileSync(path.join(SP, "r1.html"), "utf8");
let p2 = fs.readFileSync(path.join(SP, "r2.js"), "utf8");
const p3 = fs.readFileSync(path.join(SP, "r3.js"), "utf8");
const data = JSON.parse(fs.readFileSync(path.join(SP, "demo-data.json"), "utf8"));

/* 繁簡對照表與「錯誤提示查詢器」共用同一份（pairs.txt 是唯一來源） */
...
console.log("功能:", data.i.length, " 平台:", data.p.length, " 繁簡對照:", m.size, "組");
